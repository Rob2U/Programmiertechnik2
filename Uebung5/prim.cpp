...
void prim(Graph& graph)
{
    ...
    Vertex* minVertex = &graph.vertices[0];
    
    while (&(*graph.vertices.end()) != (minVertex = &(*std::find_if(graph.vertices.begin(), graph.vertices.end(), [](Vertex v) {return !(v.processed);})))) {
       
        //find vertex with lowest key
        for (int j = 0; j < graph.vertex_count; j++) {
            if(!graph.vertices[j].processed){
                if ((*minVertex).key > graph.vertices[j].key) {
                    minVertex = &graph.vertices[j];
                }
        ...
        //check if new edge found
        if ((*minVertex).parent_index != Vertex::undefined) {
            graph.mst.push_back({{(*minVertex).parent_index,(*minVertex).index}, (*minVertex).key});
        }
        
        //update parent_index and key
        std::for_each(graph.edges.begin(), graph.edges.end(), [&minVertex, &graph](Edge e){
            if (e.connected_vertices[0] == (*minVertex).index) {
                if (!(graph.vertices[e.connected_vertices[1]].processed) && e.weight < graph.vertices[e.connected_vertices[1]].key) {
                    graph.vertices[e.connected_vertices[1]].parent_index = (*minVertex).index;
                    graph.vertices[e.connected_vertices[1]].key = e.weight;
                }
            }
            if (e.connected_vertices[1] == (*minVertex).index) {
                if (!(graph.vertices[e.connected_vertices[0]].processed) && e.weight < graph.vertices[e.connected_vertices[0]].key) {
                    graph.vertices[e.connected_vertices[0]].parent_index = (*minVertex).index;
                    graph.vertices[e.connected_vertices[0]].key = e.weight;
                }
            }
